...
const jadibot = async (conn, m, from) => {
  ...
  const {
    state,
    saveCreds
  } = await useMultiFileAuthState(`./database/clone/${m.sender.split("@")[0]}`)
  try {
    async function connectToWhatsApp() {
      ...
      client[from].ev.on('messages.upsert', async chatUpdate => {
        try {
          ...
          require('./case')(client[from], m, chatUpdate, store)
        } catch (err) {
          console.log(err)
        }
      })
...
async function stopjadibot(conn, m, from) {
  ...
  delete client[from]
  fs.unlinkSync(`./database/clone/${from.split("@")[0]}`)
}
